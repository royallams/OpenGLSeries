...
/* This examples is a simple example on Interpolation
What we are trying to draw is a simple triangle that utilizes
a fragmant shader to take colors from vertex shaders..
*/
...
//CLAMP :- if the value is not between 0 to 1 then make it positive..
// Since we are not using individual pos.x ,pos.y etc here we can optimize by directly using vec4(pos, 1.0)
// in the above vertex shader
...
int main()
{
    ...
    while(!glfwWindowShouldClose(mainwindow))
    {
        ...
        //L6_Step 1:- Disble the translation . We want to just have a triangle that stays  on the centra.

//        model = glm::translate(model,glm::vec3(triOffset, 0.0f,0.0f));// Only changing the X values .Applying translation.
//        model = glm::rotate(model, curAngle*toRadians, glm::vec3(0.0f,0.0f,1.0f));

        //Using the current size to dynamically apply the changes
        //L6_Step 2:- We will not scale it incrementally ..Lets keep it constant value..
        model = glm::scale(model, glm::vec3(0.4f,0.4f,1.0f));//Scaling is usually done at last not to affect the trans and rot
